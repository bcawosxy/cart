...
class ModelAffiliateAffiliate extends Model {
	...
	public function addTransaction($affiliate_id, $amount = '', $order_id = 0) {
		...
		if ($affiliate_info) {
			...
			$this->db->query("INSERT INTO " . DB_PREFIX . "affiliate_transaction SET affiliate_id = '" . (int)$affiliate_id . "', order_id = '" . (float)$order_id . "', description = '" . $this->db->escape($this->language->get('text_order_id') . ' #' . $order_id) . "', amount = '" . (float)$amount . "', date_added = '".INSERTTIME."'");
