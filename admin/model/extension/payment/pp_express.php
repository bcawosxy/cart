...
class ModelExtensionPaymentPPExpress extends Model {
	...
	public function editPayPalOrderStatus($order_id, $capture_status) {
		$this->db->query("UPDATE `" . DB_PREFIX . "paypal_order` SET `capture_status` = '" . $this->db->escape($capture_status) . "', `date_modified` = '".INSERTTIME."' WHERE `order_id` = '" . (int)$order_id . "'");
	}
