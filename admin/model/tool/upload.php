...
class ModelToolUpload extends Model {
	public function addUpload($name, $filename) {
		...
		$this->db->query("INSERT INTO `" . DB_PREFIX . "upload` SET `name` = '" . $this->db->escape($name) . "', `filename` = '" . $this->db->escape($filename) . "', `code` = '" . $this->db->escape($code) . "', `date_added` = NOW() + INTERVAL 15 HOUR");
