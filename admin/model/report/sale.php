...
class ModelReportSale extends Model {
	...
	public function getTotalOrdersByDay() {
		...
		$query = $this->db->query("SELECT COUNT(*) AS total, HOUR(date_added) AS hour FROM `" . DB_PREFIX . "order` WHERE order_status_id IN(" . implode(",", $implode) . ") AND DATE(date_added) = DATE('".INSERTTIME."') GROUP BY HOUR(date_added) ORDER BY date_added ASC");
	...
	public function getTotalOrdersByYear() {
		...
		$query = $this->db->query("SELECT COUNT(*) AS total, date_added FROM `" . DB_PREFIX . "order` WHERE order_status_id IN(" . implode(",", $implode) . ") AND YEAR(date_added) = YEAR('".INSERTTIME."') GROUP BY MONTH(date_added)");
